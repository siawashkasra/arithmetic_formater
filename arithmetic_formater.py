def arithmetic_arranger(problems):
    ...
    for problem in problems_list:
        if len(problem[0]) < len(problem [2]):
            for align, p in zip('<', problem):
                header += '{0:{fill}{align}{width}}'.format(problem[0].rjust(len(problem[1]) + len(problem[2]) + 1), fill='*', align=align, width= 2 + len(problem[2]) + 2)
                bottom += '{0:{fill}{align}{width}}'.format(problem[1].ljust(len(problem[1]) + 1) + problem[2], fill="*", align=align, width=2 + len(problem[2]) + 2)
                dashes = len(bottom) * '-'
        # if len(problem[0]) == len(problem [2]):
        #     for align, p in zip('<', problem):
    ...
    return arranged[0]
print(arithmetic_arranger(["32 + 546", "3801 - 2", "45 + 43", "123 + 49"]))